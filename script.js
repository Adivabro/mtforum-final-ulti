...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (input) {
    input.addEventListener("keypress", (e) => {
      if (e.key === "Enter") {
        ...
        setTimeout(() => {
          if (
            userMsg.toLowerCase().includes("teknikal") ||
            userMsg.toLowerCase().includes("indikator")
          ) {
            appendMsg(
              "🤖",
              "📉 Teknikal hanya pola. FUNDAMENTAL menjelaskan *mengapa* harga bergerak."
            );
          } else {
            appendMsg(
              "🤖",
              "🔍 Fokus pada FUNDAMENTAL: inflasi, kebijakan moneter, supply-demand, dan geopolitik."
            );
          }
        }, 500);
      }
  ...
  const translations = {
    id: {
      ...
      "why-text":
        "Indikator hanya membaca pola masa lalu. FUNDAMENTAL menjelaskan alasan sebenarnya pergerakan harga.",
    ...
    en: {
      ...
      "why-text":
        "Indicators only read past patterns. FUNDAMENTAL explains the real reasons behind price moves.",
      "insight-title": "Market Insight",
    ...
    ru: {
      ...
      "why-text":
        "Индикаторы читают прошлое. Фундамент объясняет настоящие причины движения цен.",
    ...
    ar: {
      ...
      "why-text":
        "المؤشرات تقرأ الماضي فقط. الأساسيات تشرح الأسباب الحقيقية لتحرك الأسعار.",
